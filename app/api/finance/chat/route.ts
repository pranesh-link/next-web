      ...
      try {
        ...
        for (let step = 0; step < 5; step++) {
          const completion = await client.chat.completions.create({
            ...
            messages: [{ role: "system", content: systemPrompt }, ...current] as Parameters<typeof client.chat.completions.create>[0]["messages"],
          ...
          if (choice.finish_reason === "tool_calls" && msg.tool_calls?.length) {
            ...
            // Final answer — stream it word by word for a streaming feel
            const content = msg.content ?? "";
            const chunks = content.match(/\S+\s*/g) ?? [content];
            for (const chunk of chunks) {
              send({ type: "text_delta", delta: chunk });
            }
            break;
          }
        }
