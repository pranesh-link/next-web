...
export async function getAccountActivity(accountId: string, cursor?: string) {
  ...
  try {
    ...
    items.sort((a, b) => {
      const diff = new Date(b.date as unknown as string).getTime() - new Date(a.date as unknown as string).getTime();
    ...
    if (cursorDate && cursorId) {
      ...
      const idx = filtered.findIndex(
        (item) => new Date(item.date as unknown as string).getTime() === cursorTime && item.id === cursorId,
      );
      if (idx !== -1) {
        filtered = filtered.slice(idx + 1);
      } else {
        filtered = filtered.filter((item) => new Date(item.date as unknown as string).getTime() < cursorTime);
      }
    }
