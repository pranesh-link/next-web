...
export async function registerHealthRoutes(app: FastifyInstance) {
  ...
  app.post("/body-metrics", { preHandler: requireAuth }, async (req, reply) => {
    const { userId } = req as unknown as AuthReq & typeof req;
    const [row] = await db.insert(bodyMetrics).values({ userId, ...(req.body as any) }).returning();
  ...
  app.post("/nutrition", { preHandler: requireAuth }, async (req, reply) => {
    const { userId } = req as unknown as AuthReq & typeof req;
    const [row] = await db.insert(nutritionLogs).values({ userId, ...(req.body as any) }).returning();
    return reply.code(201).send({ success: true, data: row });
  });
  ...
  app.post("/sleep", { preHandler: requireAuth }, async (req, reply) => {
    const { userId } = req as unknown as AuthReq & typeof req;
    const [row] = await db.insert(sleepLogs).values({ userId, ...(req.body as any) }).returning();
    return reply.code(201).send({ success: true, data: row });
  });
  ...
  app.post("/exercise", { preHandler: requireAuth }, async (req, reply) => {
    const { userId } = req as unknown as AuthReq & typeof req;
    const [row] = await db.insert(exerciseLogs).values({ userId, ...(req.body as any) }).returning();
    return reply.code(201).send({ success: true, data: row });
  });
  ...
  app.post("/habits", { preHandler: requireAuth }, async (req, reply) => {
    const { userId } = req as unknown as AuthReq & typeof req;
    const [row] = await db.insert(habitLogs).values({ userId, ...(req.body as any) }).returning();
    return reply.code(201).send({ success: true, data: row });
  });
}
